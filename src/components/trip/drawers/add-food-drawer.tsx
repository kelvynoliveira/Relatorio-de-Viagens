...
import { FoodEntry, FoodEntrySchema } from '@/lib/models';
...
export default function AddFoodDrawer({ open, onOpenChange, tripId, initialData }: AddFoodDrawerProps) {
    ...
    const onSubmit = async (data: FoodEntry) => {
        ...
        let newEntries = [...(trip.foodEntries || [])];
        if (initialData) {
        ...
        try {
            await updateTrip(tripId, { foodEntries: newEntries });
            toast.success(initialData ? 'Alimentação atualizada!' : 'Alimentação registrada!');
            onOpenChange(false);
        } catch (error) {
            console.error('Error in AddFoodDrawer:', error);
        } finally {
            setIsSubmitting(false);
        }
    };
        ...
        <Drawer open={open} onOpenChange={onOpenChange}>
            <DrawerContent className="bg-black/60 backdrop-blur-2xl border-t border-white/10 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] outline-none">
                <div className="mx-auto w-full max-w-sm flex flex-col h-full max-h-[85vh]">
                    <DrawerHeader>
                        <DrawerTitle className="text-2xl font-bold text-white tracking-tight">{initialData ? 'Editar Alimentação' : 'Registrar Alimentação'}</DrawerTitle>
                        <DrawerDescription className="text-muted-foreground/80">{initialData ? 'Altere os dados da alimentação.' : 'Valor gasto com refeições.'}</DrawerDescription>
                    </DrawerHeader>
                            ...
                            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                                ...
                                <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel className="text-xs font-bold text-red-400 uppercase tracking-wider flex items-center gap-2">
                                                    <span className="w-1.5 h-1.5 bg-red-400 rounded-full animate-pulse" />
                                                    Valor (R$)
                                                </FormLabel>
                                                <FormControl>
                                                    <div className="relative group">
                                                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground group-focus-within:text-rose-500 transition-colors">R$</span>
                                                        <CurrencyInput
                                                            placeholder="0,00"
                                                            value={field.value}
                                                            onChange={field.onChange}
                                                            className="text-lg font-bold pl-9 bg-white/5 border-white/10 focus:border-rose-500/50 text-white h-12"
                                                        />
                                                    </div>
                                                </FormControl>
                                                <FormMessage />
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Local / Restaurante</FormLabel>
                                            <FormControl>
                                                <Input placeholder="Ex: Restaurante da Esquina" {...field} value={field.value ?? ''} className="bg-white/5 border-white/10 focus:border-primary/50 text-white" />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input placeholder="Almoço dia 10/01" {...field} value={field.value ?? ''} className="bg-white/5 border-white/10 focus:border-primary/50 text-white" />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
